        ...
        <section className="bg-slate-100">
            <div className="py-8 px-4">
                ...
                <div class="space-y-8 lg:grid lg:grid-cols-3 sm:gap-6 xl:gap-10 lg:space-y-0">
                    {packages.map((p) => (
                        <div key={p.id} class="flex flex-col p-6 mx-auto max-w-lg text-center text-gray-900 bg-black rounded-lg border border-black shadow dark:border-slate-200 xl:p-8 dark:bg-slate-200 dark:text-gray-900">
                            <h3 class="mb-4 text-2xl font-semibold">{p.name}</h3>
                            <p class="font-light text-gray-800 sm:text-lg dark:text-gray-800">Best option for personal use & for your next project.</p>
                            <div class="flex justify-center items-baseline my-8">
                                <span class="mr-2 text-5xl font-extrabold">${p.price}</span>
                                <span class="text-gray-800 dark:text-gray-800">/{p.credits} Credits</span>
                            </div>
